...
const MakeOfferModal = ({
	...
	const diffFromAsking =
		offerNum > 0
			? (((offerNum - askingPrice) / askingPrice) * 100).toFixed(1)
			: null;
	...
	const handleSubmit = async () => {
		...
		try {
			...
			analytics.offerMade(listing.id, offerAmount, address || "unknown");
			...
			<DialogContent className="bg-card border-border max-w-lg max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					<div className="flex items-center gap-2">
						...
						<Badge variant="muted" className="font-mono-numbers">
							{listing.id}
						</Badge>
					</div>
					<DialogDescription>
						Submit your offer for this position
					</DialogDescription>
				</DialogHeader>
					...
					<div className="rounded-lg bg-secondary/50 p-3 space-y-1.5 text-sm">
						...
						<div className="flex justify-between">
							...
							<span className="font-mono-numbers">
								{listing.loan.collateralAmount}{" "}
								{listing.loan.collateralToken}
							</span>
						</div>
						<div className="flex justify-between">
							...
							<div className="mt-2">
								{isBelowAsking ? (
									<span className="flex items-center gap-1.5 text-sm text-warning">
										...
										Your offer: {offerNum.toLocaleString()}{" "}
										{listing.loanToken} ({diffFromAsking}%
										below asking)
									</span>
								) : (
									<span className="flex items-center gap-1.5 text-sm text-success">
										<CheckCircle2 className="h-4 w-4" />
										At or above asking price — consider
										buying now
									</span>
								)}
							</div>
