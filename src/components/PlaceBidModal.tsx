...
const PlaceBidModal = ({ open, onOpenChange, auction }: PlaceBidModalProps) => {
	...
	// Get USDC balance - use demo balance if in demo mode, otherwise fetch from blockchain
	const { data: balance } = useBalance({
		address,
		// Use USDC address for Arbitrum Sepolia
		token: "0x75faf114eafb1BDbe2F0316DF893fd58CE46AA4d",
		query: {
			enabled: !isDemoMode && !!address, // Only fetch if not in demo mode
		},
	});

	// Use demo balance if in demo mode, otherwise use fetched balance or default to 100k
	const userBalance = isDemoMode
		? parseFloat(demoBalances[auction.loanToken] || "0")
		: balance
			? parseFloat(balance.formatted)
			: 100000; // Mock 100k for demo

	const currentBidNum = auction.currentBid
		? parseFloat(auction.currentBid)
		: parseFloat(auction.maxRepayment);
	...
	const validation = useMemo(() => {
		...
		if (bidNum < loanAmountNum) {
			return {
				valid: false,
				message: `Bid must be at least ${loanAmountNum.toLocaleString()} ${auction.loanToken}`,
			};
		}
		if (bidNum >= currentBidNum) {
			return {
				valid: false,
				message: `Bid must be lower than current bid (${currentBidNum.toLocaleString()})`,
			};
		}
	...
	const interest =
		bidNum >= loanAmountNum
			? (((bidNum - loanAmountNum) / loanAmountNum) * 100).toFixed(1)
			: null;
	const profit =
		bidNum >= loanAmountNum ? (bidNum - loanAmountNum).toFixed(0) : null;
	const roi =
		bidNum >= loanAmountNum
			? (((bidNum - loanAmountNum) / bidNum) * 100).toFixed(1)
			: null;
				...
				<div className="space-y-5">
						...
						{auction.currentBidder && (
							<div className="flex justify-between">
								...
								<span className="font-mono-numbers text-xs">
									{auction.currentBidder.slice(0, 6)}...
					...
					<div>
						...
						<p className="text-xs text-muted-foreground mt-1.5">
							Lower = Better for Borrower | Min:{" "}
							{loanAmountNum.toLocaleString()} {auction.loanToken}
						</p>
						{bidNum > 0 && (
							<div className="mt-2">
								{validation.valid ? (
					...
					{bidNum >= loanAmountNum && (
						<div className="rounded-lg border border-border p-4 space-y-2 text-sm">
							<div className="flex justify-between">
								...
								<span className="font-mono-numbers text-accent">
									~{interest}% APR
								</span>
							</div>
							<div className="flex justify-between">
						...
						<div className="rounded-lg border border-warning/30 bg-warning/5 p-4 flex items-start gap-3">
							...
							<div>
								<p className="text-sm font-medium">
									First, approve {auction.loanToken} spending
								</p>
								<Button
									variant="outline"
									size="sm"
					...
					{isApproved && (
						<div className="rounded-lg border border-success/30 bg-success/5 p-3 flex items-center gap-2">
							...
							<span className="text-sm text-success">
								{auction.loanToken} approved for spending
							</span>
						</div>
					)}
				</div>
