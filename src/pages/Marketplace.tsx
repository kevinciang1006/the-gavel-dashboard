...
const Marketplace = () => {
	...
	const [positionType, setPositionType] = useState<"borrower" | "lender">(
		"lender",
	);
	...
	// Modal state
	const [buyListing, setBuyListing] = useState<MarketplaceListing | null>(
		null,
	);
	const [offerListing, setOfferListing] = useState<MarketplaceListing | null>(
		null,
	);
	...
	const userLoans = useMemo(() => {
		...
		return loans.filter((l) => {
			...
			const isActive =
				l.status === "active" || l.status === "grace_period";
	...
	const filteredListings = useMemo(() => {
		return activeListings.filter((l) => {
			if (tabFilter === "borrower" && l.nftType !== "borrower")
				return false;
			if (tabFilter === "lender" && l.nftType !== "lender") return false;
			if (
				collateralFilter !== "All" &&
				l.loan.collateralToken !== collateralFilter
			)
				return false;
			if (
				search &&
				!l.id.includes(search) &&
				!l.loan.collateralAmount
					.toLowerCase()
	...
	const positionValue = selectedLoan
		? parseFloat(selectedLoan.loanAmount)
		: 0;
	const priceDiff =
		askingPriceNum > 0 && positionValue > 0
			? (
					((askingPriceNum - positionValue) / positionValue) *
					100
				).toFixed(1)
			: null;
				...
				>
					...
					<div className="flex-1">
						...
						<p className="text-xs text-muted-foreground">
							Connect to list positions or make purchases
						</p>
					</div>
					<Button
						variant="outline"
						...
						<CardContent className="space-y-5">
							...
							<div>
								<p className="text-sm font-medium mb-2">
									Step 1: Select Position to Sell
								</p>
								<Select
									value={positionType}
									onValueChange={(v) =>
										setPositionType(
											v as "borrower" | "lender",
										)
									}
								>
							...
							{selectedLoanId && askingPriceNum > 0 && (
								...
								>
									...
									<Select
										value={listingExpiry}
										onValueChange={setListingExpiry}
									>
							...
							{filteredListings.length > 0 ? (
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									{filteredListings.map((listing, i) => {
										const loanValue = parseFloat(
											listing.loan.loanAmount,
										);
