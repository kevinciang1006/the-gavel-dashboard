...
const kpis = [
	...
	},
	{
		...
		icon: Percent,
		sparkline: [
			12, 11.5, 11, 10.8, 10.5, 10.2, 10, 9.8, 9.7, 9.6, 9.5, 9.5,
		],
	},
];
...
// Removed hardcoded recentTransactions
...
const actionColors: Record<string, string> = {
	...
	"Auction Finalized": "success",
	"Collateral Claimed": "destructive",
	"Position Bought": "success",
	"Position Sold": "warning",
	"Offer Made": "accent",
	"NFT Minted": "default",
};
...
const Analytics = () => {
	...
	const transactions = useAnalyticsStore((state) => state.transactions);
	...
	const dynamicKpis = useMemo(() => {
		...
		const activeLoansCount = loans.filter(
			(l) => l.status === "active" || l.status === "grace_period",
		).length;
		...
		<div className="container py-8">
			...
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Real-time lending market insights
					</p>
				</div>
							...
							<CardContent className="p-5">
								...
								<div className="flex items-end justify-between">
									<div>
										...
										<div
											className={`flex items-center gap-1 text-xs mt-1 ${kpi.positive ? "text-success" : "text-destructive"}`}
										>
											{kpi.positive ? (
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
				...
				<Card className="border-border bg-card">
					<CardHeader>
						<CardTitle className="text-base">
							Borrower vs Lender Volume
						</CardTitle>
					</CardHeader>
					<CardContent>
						<ChartContainer
							config={borrowLendConfig}
							className="h-[280px]"
						>
			...
			<Card className="border-border bg-card mb-8">
				<CardHeader>
					...
					<p className="text-xs text-muted-foreground">
						Market-driven rates for different loan terms
					</p>
				</CardHeader>
				<CardContent>
					<ChartContainer
								...
								{recentTransactions.map((tx, i) => (
									...
									>
										...
										<TableCell className="text-right hidden sm:table-cell">
											<a
												href="#"
												className="text-accent text-xs flex items-center gap-1 justify-end hover:underline"
											>
												View{" "}
