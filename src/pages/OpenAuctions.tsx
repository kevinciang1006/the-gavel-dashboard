...
const OpenAuctions = () => {
	...
	const updateAuctionStatuses = useAuctionStore(
	...
	const [viewBidsAuction, setViewBidsAuction] = useState<Auction | null>(
		null,
	);
	...
	const filtered = useMemo(() => {
		return auctions.filter((a) => {
			if (
				...
				a.collateralToken !== collateralFilter
			)
				return false;
			if (
				search &&
				!a.id.includes(search) &&
				!a.collateralAmount.toLowerCase().includes(search.toLowerCase())
			)
				return false;
			...
			<div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
				...
				<div className="rounded-lg border border-border bg-card p-4">
					...
					<p className="text-2xl font-bold font-mono-numbers text-warning">
						{
							auctions.filter((a) => a.status === "ending_soon")
								.length
						}
					</p>
				</div>
				...
				<div className="rounded-lg border border-border bg-card p-4">
					...
					<p className="text-2xl font-bold font-mono-numbers text-accent">
						...
						{auctions
							.reduce(
								(sum, a) => sum + parseFloat(a.loanAmount),
								0,
							)
							.toLocaleString()}
					</p>
								...
								return (
									...
									>
										...
										<TableCell className="hidden sm:table-cell">
											<span
												className={`flex items-center gap-1.5 font-mono-numbers text-sm ${
													isEndingSoon
														? "text-warning"
														: ""
												}`}
