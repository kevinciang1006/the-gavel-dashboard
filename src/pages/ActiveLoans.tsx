...
function getLoanDisplayStatus(loan: Loan): { status: string; color: string; progressColor: string; progress: number } {
  const now = Date.now();
  ...
  if (loan.status === "repaid") {
    return { status: "Repaid", color: "text-success", progressColor: "bg-success", progress: 100 };
  }
  if (loan.status === "defaulted") {
    return { status: "Defaulted", color: "text-destructive", progressColor: "bg-destructive", progress: 100 };
  }
  if (now > loan.gracePeriodEnd) {
    return { status: "Overdue", color: "text-destructive", progressColor: "bg-destructive", progress: 100 };
  }
  if (now > loan.maturityTime) {
    return { status: "Grace Period", color: "text-warning", progressColor: "bg-warning", progress: 100 };
  }
  if (loan.maturityTime - now < 24 * 60 * 60 * 1000) {
    return { status: "Approaching", color: "text-warning", progressColor: "bg-warning", progress };
  }
  return { status: "Healthy", color: "text-success", progressColor: "bg-success", progress };
}
...
const ActiveLoans = () => {
  ...
  const updateLoanStatuses = useLoanStore((state) => state.updateLoanStatuses);

  const [roleFilter, setRoleFilter] = useState<"all" | Role>("all");
  const [expandedLoan, setExpandedLoan] = useState<string | null>(null);
  const [selectedLoan, setSelectedLoan] = useState<Loan | null>(null);
  const [actionType, setActionType] = useState<"repay" | "claim" | null>(null);
  ...
  const filtered = useMemo(() => {
    return activeLoans.filter((l) => {
      ...
      if (roleFilter === "borrower") {
        return address && l.borrower.toLowerCase() === address.toLowerCase();
      }
      if (roleFilter === "lender") {
        return address && l.lender.toLowerCase() === address.toLowerCase();
      }
      return true;
    });
  ...
  const stats = useMemo(() => {
    const totalLoaned = filtered.reduce((sum, l) => sum + parseFloat(l.loanAmount), 0);
    const totalInterest = filtered.reduce((sum, l) => sum + (parseFloat(l.repaymentAmount) - parseFloat(l.loanAmount)), 0);
  ...
  const getUserRole = (loan: Loan): Role | null => {
    ...
    if (loan.borrower.toLowerCase() === address.toLowerCase()) return "borrower";
    if (loan.lender.toLowerCase() === address.toLowerCase()) return "lender";
    return null;
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-8">
        ...
        {!isConnected && (
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">Connect to view your loans</p>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {[
            { label: "Total Loaned", value: stats.totalLoaned, icon: DollarSign },
            { label: "Interest Earned", value: stats.totalInterest, icon: TrendingUp },
            { label: "Avg APR", value: stats.avgApr, icon: Percent },
          ].map((card, i) => (
            <motion.div
              key={card.label}
        ...
        {filtered.length > 0 ? (
            ...
            <Table>
              ...
              <TableBody>
                {filtered.map((loan) => {
                  ...
                  const timeLeft = formatTimeLeft(loan.maturityTime);
                  const apr = ((parseFloat(loan.repaymentAmount) - parseFloat(loan.loanAmount)) / parseFloat(loan.loanAmount) * 100).toFixed(1);
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge variant={userRole === "borrower" ? "default" : "accent"}>
                            {userRole === "borrower" ? "Borrower" : userRole === "lender" ? "Lender" : "—"}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <span className="flex items-center gap-2">
                            <span className="text-lg">{loan.collateralToken === "WBTC" ? "₿" : "Ξ"}</span>
                            <span className="font-mono-numbers text-sm">{loan.collateralAmount} {loan.collateralToken}</span>
                          </span>
                        </TableCell>
                        ...
                        <TableCell className="hidden lg:table-cell">
                          ...
                          <Badge variant="accent" className="ml-2 text-[10px]">{apr}%</Badge>
                        </TableCell>
                        <TableCell>
                          <div className="space-y-1.5 min-w-[120px]">
                            ...
                            <span className={`text-xs font-medium ${displayStatus.color}`}>{timeLeft}</span>
                          </div>
                        </TableCell>
                        <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                          {userRole === "borrower" && loan.status !== "repaid" ? (
                            <Button variant="gradient" size="sm" onClick={() => handleRepay(loan)}>
                              Repay Loan
                            </Button>
                          ) : userRole === "lender" && (loan.status === "overdue" || Date.now() > loan.gracePeriodEnd) ? (
                            <Button variant="accent" size="sm" onClick={() => handleClaim(loan)}>
                              Claim
                            </Button>
                          ) : (
                          ...
                          <TableCell colSpan={8} className="p-4">
                            ...
                            >
                              <div>
                                ...
                                <span className="font-medium">{new Date(loan.startTime).toLocaleDateString()}</span>
                              </div>
                              <div>
                                <span className="text-muted-foreground block text-xs mb-0.5">Maturity Date</span>
                                <span className="font-medium">{new Date(loan.maturityTime).toLocaleDateString()}</span>
                              </div>
                              <div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center rounded-xl border border-border bg-card">
            ...
            <h3 className="text-lg font-semibold mb-1">No active loans</h3>
            <p className="text-sm text-muted-foreground">
              {isConnected ? "Your loans will appear here" : "Connect wallet to view your loans"}
            </p>
          </div>
        )}
      </main>
      ...
      {/* Confirmation Dialog */}
      <Dialog open={!!selectedLoan && !!actionType} onOpenChange={(open) => !open && setSelectedLoan(null)}>
        <DialogContent className="bg-card border-border max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl">
              {actionType === "repay" ? "Repay Loan" : "Claim Collateral"}
            </DialogTitle>
            <DialogDescription>
              {actionType === "repay"
            ...
            <div className="space-y-4">
              <div className="rounded-lg bg-secondary/50 p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono-numbers">{selectedLoan.id}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Collateral</span>
                  <span className="font-mono-numbers">{selectedLoan.collateralAmount} {selectedLoan.collateralToken}</span>
                </div>
                {actionType === "repay" && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Repayment Amount</span>
                    <span className="font-mono-numbers font-bold">
                      {parseFloat(selectedLoan.repaymentAmount).toLocaleString()} {selectedLoan.loanToken}
                    </span>
                  </div>
                )}
              </div>
              ...
              {actionType === "repay" && (
                <div className="rounded-lg border border-warning/30 bg-warning/5 p-3 flex items-start gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    This will transfer {parseFloat(selectedLoan.repaymentAmount).toLocaleString()} {selectedLoan.loanToken} from your wallet
                  </p>
                </div>
              )}
            </div>
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setSelectedLoan(null)} disabled={isProcessing}>
              Cancel
            </Button>
            <Button variant="gradient" onClick={confirmAction} disabled={isProcessing}>
              {isProcessing ? (
